...
namespace PdfiumViewer.Demo
{
    public sealed class ShellItem : IEnumerable, IDisposable, IComparable
    {
        ...
        {
            IntPtr ptrParent;
            ShellAPI.SHBindToParent(pidl, ref IID_IShellFolder, out ptrParent, out pidlRelative);
            ...
            IShellFolder folder = (IShellFolder)obj;
            if (ptrParent != IntPtr.Zero)
            {
                try
                {
                    Marshal.Release(ptrParent);
                }
                catch (Exception) { }
                finally
                {
                    ptrParent = IntPtr.Zero;
                }
            }
            ...
            folder.BindToObject(
                                           ...
                                           ref ShellAPI.IID_IShellFolder,
                                           out shellFolderPtr);
            obj = System.Runtime.InteropServices.Marshal.GetTypedObjectForIUnknown(shellFolderPtr, shellFolderType);
